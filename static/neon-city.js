...
function frame(now){const dt=Math.min(80,now-(last||now));last=now;if(!document.hidden&&!paused&&document.querySelector('.motion-control')?.getAttribute('aria-pressed')!=='false'&&!document.querySelector('dialog')?.open){t+=dt/1000;acc+=dt;if(acc>50){draw();acc=0}}requestAnimationFrame(frame)}
