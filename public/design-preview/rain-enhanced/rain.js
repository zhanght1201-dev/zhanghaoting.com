...
ctx.globalAlpha=1;c.dataset.tick=Math.round(t*100);
}
...
const status=document.querySelector('#rain-status');document.querySelectorAll('[data-level]').forEach(button=>button.onclick=()=>{enhanced=button.dataset.level!=='0';realRain=button.dataset.level==='2';document.querySelectorAll('[data-level]').forEach(b=>b.setAttribute('aria-pressed',String(b===button)));status.textContent=realRain?'写实像素雨：细雨分层 · 连续斜向坠落 · 落水溅点与细涟漪':enhanced?'增强效果：楼灯逐层亮灭 · 像素倒影联动 · 阵雨与涟漪':'当前线上效果：低频窗灯 · 细雨 · 轻微倒影';if(ready)draw()});document.querySelector('#rain-pause').onclick=e=>{paused=!paused;if(ready)draw();e.currentTarget.setAttribute('aria-pressed',String(paused));e.currentTarget.textContent=paused?'播放背景':'暂停背景'};document.querySelector('#rain-reset').onclick=()=>{t=0;paused=false;document.querySelector('#rain-pause').textContent='暂停背景';document.querySelector('#rain-pause').setAttribute('aria-pressed','false')};document.querySelector('#rain-solo').onclick=e=>{const solo=document.body.classList.toggle('rain-solo');e.currentTarget.setAttribute('aria-pressed',String(solo));e.currentTarget.textContent=solo?'返回首页合成':'仅看背景'};
})();
