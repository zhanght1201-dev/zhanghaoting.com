(() => {
 ...
 function piece(stage,points){const el=document.createElement('div');el.className='story-piece';el.style.clipPath=polygon(points);stage.append(el);return el}
 ...
 function overlay(index){
  ...
  screen.innerHTML=`<div class="transition-wash"></div><p class="transition-heading">${headings[index]}</p><div class="transition-sheet" aria-hidden="true"></div><span class="transition-step" aria-live="polite">画格切入 · 拼合漫画页</span><button class="transition-skip" type="button">跳过转场 →</button>`;
 ...
 function load(index){return new Promise((resolve,reject)=>{const img=new Image();img.onload=resolve;img.onerror=reject;img.src=`/images/comic/story-${slugs[index]}-final.png`})}
 function depart(index,url){
  ...
  s.skip=()=>go(false);s.later(()=>go(false),6500);
  load(index).then(()=>{if(active!==s)return;
